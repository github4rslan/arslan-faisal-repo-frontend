...
const Chatbot = () => {
  ...
  const [loading, setLoading] = useState(false);  // To handle the loading state while waiting for bot response
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const formattedMessage = formatMessage(chatbotMessage);

      // Add the bot's message to the chat
      setMessages((prevMessages) => [
        ...prevMessages,
        newMessage,  // User message
        { role: 'bot', text: formattedMessage }  // Bot's response
      ]);
    } catch (error) {
  ...
  const formatMessage = (message) => {
    ...
    // Split the message into paragraphs based on newline characters
  ...
  return (
    <div style={styles.chatContainer}>
      ...
      <div style={styles.chatBox}>
        ...
        {/* Show loading message while waiting for the bot's response */}
        {loading && (
