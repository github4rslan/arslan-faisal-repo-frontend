...
export default function Login() {
    ...
    getRedirectResult(auth)
      .then(async (result) => {
        if (result?.user) {
          ...
          // Google user doesn’t come with _id → fallback to uid
          const normalizedUser = { id: user.uid, name: user.displayName, email: user.email };
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ✅ normalize backend user before saving
