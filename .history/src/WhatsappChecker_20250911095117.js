...
export default function WhatsAppChecker() {
  ...
  const [endpoint, setEndpoint] = useState("osint");
  ...
  // ✅ Render results per endpoint
  const renderResult = () => {
    if (!result?.data) return <p>No data found for this number.</p>;
      ...
      case "devices":
        return (
          <>
            ...
            <p><strong>Linked Devices:</strong> {result.data.devices}</p>
          </>
        );
      ...
      case "business":
        return (
          <>
            ...
            <p><strong>Business Type:</strong> {result.data.isBusiness}</p>
            <p><strong>Verified Name:</strong> {result.data.verifiedName}</p>
          </>
        );

      default:
        return <p>Unsupported endpoint.</p>;
    }
  };
