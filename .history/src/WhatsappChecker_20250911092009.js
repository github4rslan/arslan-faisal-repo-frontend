  ...
  const renderResult = () => {
    ...
    switch (result.type) {
      case "business":
        return (
          <>
            <p><strong>Exists:</strong> {result.data?.exists ? "Yes ✅" : "No ❌"}</p>
            <p><strong>Type:</strong> {result.data?.type || "Unknown"}</p>
            ...
            {result.data?.about && <p><strong>About:</strong> {result.data.about}</p>}
