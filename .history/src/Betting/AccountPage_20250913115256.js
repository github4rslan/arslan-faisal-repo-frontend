...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    // ✅ Listen for localStorage updates (Deposit/Withdraw triggers)
  ...
  if (!user) {
    return <p style={{ textAlign: "center", marginTop: "50px", color: "red" }}>❌ No user info found. Please login.</p>;
  }

  const winRate = user.totalBets > 0 ? ((user.betsWon / user.totalBets) * 100).toFixed(1) : 0;
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ marginTop: "30px" }}>
        ...
        <button onClick={fetchUser} style={{ ...buttonStyle, backgroundColor: "#28a745", marginLeft: "10px" }}>
          🔄 Refresh Balance
        </button>
      </div>
    </div>
