...
export default function EvenOddPage() {
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Live Balance */}
      <p style={styles.balanceText}>💰 Balance: PKR {balance}</p>
      ...
      <div style={styles.card}>
        ...
        <button onClick={handlePlay} style={styles.mainButton} disabled={loading}>
        ...
        {history.length === 0 ? (
          ...
          <ul style={{ listStyle: "none", padding: 0 }}>
            {history.map((h, idx) => (
              ...
              >
                [{new Date(h.time).toLocaleString()}] (Number: {h.number}) {h.amount > 0 ? `+${h.amount}` : h.amount} PKR
              </li>
            ))}
          </ul>
        )}
      ...
      {/* Back Button */}
      <button onClick={() => navigate("/betting")} style={styles.backButton}>
